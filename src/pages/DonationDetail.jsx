...
import { Clock, MapPin, CalendarIcon, Package, ChevronLeft, Phone, Mail } from 'lucide-react';
...
const getDonationById = (id) => {
  ...
  return {
    ...
    description: "Assorted fresh vegetables and fruits including apples, oranges, carrots, and potatoes. All items are fresh and in good condition. Perfect for preparing nutritious meals.",
    ...
    imageUrl: "https://images.unsplash.com/photo-1610348725531-843dff563e2c?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1470&q=80",
...
const DonationDetail = () => {
  ...
  const handleReserveDonation = () => {
    ...
    alert('Donation reserved successfully! Please check your dashboard for pickup details.');
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-grow pt-24 pb-16">
        <div className="container mx-auto max-w-6xl px-4">
          <div className="mb-6">
            <Link to="/donations" className="inline-flex items-center text-sage-600 hover:text-sage-700 transition-colors">
          ...
          <div className="bg-white rounded-xl overflow-hidden border border-border shadow-sm animate-fade-up">
              ...
              <div className="relative h-64 md:h-80">
                ...
                <div className="absolute top-4 right-4">
                  <span className={`px-4 py-2 rounded-full text-sm font-medium ${
                    ...
                    donation.status === 'reserved' ? 'bg-blue-100 text-blue-700' : 
                    'bg-sage-100 text-sage-700'
                  }`}>
                    {isExpired && donation.status === 'available' ? 'Expired' : 
            ...
            <div className="p-6 md:p-8">
              <div className="flex flex-col md:flex-row md:items-start md:justify-between mb-6">
                <div>
                  <h1 className="text-2xl md:text-3xl font-bold mb-2">{donation.title}</h1>
                  <p className="text-muted-foreground">
                    Posted by {donation.donorName} on {formatDate(donation.createdAt)}
                  </p>
                </div>
                {!isExpired && donation.status === 'available' && user && user.role === 'orphanage' && (
                  <button 
                  ...
                  <div className="bg-sage-50 p-4 rounded-lg space-y-4">
                    ...
                    <div className="flex items-start">
                      ...
                      <div>
                        ...
                        <span>
                          {formatDate(donation.pickupTimeStart)} from {formatTime(donation.pickupTimeStart)} to {formatTime(donation.pickupTimeEnd)}
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="border-t border-sage-200 my-4 pt-4">
                      ...
                      <div className="space-y-2">
                        <div className="flex items-center">
                          ...
                          <a href={`tel:${donation.contactPhone}`} className="text-sage-600 hover:underline">
                            {donation.contactPhone}
                          </a>
                        </div>
                        <div className="flex items-center">
                          <Mail className="w-4 h-4 mr-2 text-sage-500" />
                          <a href={`mailto:${donation.contactEmail}`} className="text-sage-600 hover:underline">
              ...
              {isExpired && (
                <div className="bg-red-50 text-red-700 p-4 rounded-lg mb-6">
                  <p className="font-medium">This donation has expired and is no longer available.</p>
                </div>
              )}
              
              {donation.status === 'reserved' && (
                <div className="bg-blue-50 text-blue-700 p-4 rounded-lg mb-6">
                  <p className="font-medium">This donation has been reserved and is pending pickup.</p>
                </div>
              )}
              
              {!user && donation.status === 'available' && !isExpired && (
                <div className="bg-sage-50 p-6 rounded-lg text-center">
                  <h3 className="text-lg font-medium mb-2">Want to reserve this donation?</h3>
                  <p className="mb-4">You need to be registered as an orphanage to reserve food donations.</p>
